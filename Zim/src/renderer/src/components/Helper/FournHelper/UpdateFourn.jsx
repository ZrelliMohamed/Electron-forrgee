...
function UpdateFourn() {
  const location = useLocation();
  const initialFournData = location.state.fournData;
  ...
  useEffect(() => {
    if (initialFournData) {
      setUpdatedClient(initialFournData);
    }
  }, [initialFournData]);
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setUpdatedClient((prevFourn) => ({
      ...prevFourn,
      [name]: value,
    }));
  };
