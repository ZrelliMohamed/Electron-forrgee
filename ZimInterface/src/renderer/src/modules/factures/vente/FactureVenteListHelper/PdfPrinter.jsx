...
const { articles, client, DateFacture, netAPayer, Nbc, dateBC, exonere,totalcalcul,Numero } = factureToPrint;
 ...
 for (let i = 0; i < numIterations; i++) {
      ...
      yPos = await addHeaderSection(doc, imgData, Numero, DateFacture);
...
TVA DUE : ${(totalcalcul.netAPayer / 100 * setting.Tva).toFixed(3)}`, yPos, 170);
        }
...
TVA DUE : ${(totalcalcul.netAPayer / 100 * setting.Tva).toFixed(3)}`, yPos, 170);
  ...
  const addHeaderSection = async (doc, imgData, Numero, DateFacture) => {
  ...
  function addInvoiceDataTable(doc, totalcalcul, yPos) {
  ...
  function addNetAPayerSection(doc, netAPayer, yPos, str) {
    const rectX = 10;
    const rectY = yPos + 18;
    ...
    const secondLineText = `${netAPayer}`;
  ...
  return (
    <div className="flex-1 border border-slate-200 shadow-soft overflow-hidden">
      <iframe
        title="PDF Preview"
        ref={iframeRef}
